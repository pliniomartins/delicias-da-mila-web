...
function Pedidos() {
  ...
  const buscarPedidos = async () => {
    try {
      const res = await fetch("http://localhost:5028/api/Pedidos");
  ...
  const atualizarStatus = async (id, status) => {
    try {
      const res = await fetch(`http://localhost:5028/api/Pedidos/${id}/status`, {
        method: "PUT",
    ...
    <div style={{
      ...
      {novoPedido && (
        <div style={{
          ...
          fontSize: "15px", fontWeight: "bold", zIndex: 999,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "16px", marginBottom: "32px" }}>
          ...
          <StatCard emoji="🕐" label="Último pedido" cor="249,115,22"
            value={pedidos.length > 0
              ? new Date(pedidos[0].criadoEm).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })
              : "--:--"} />
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: "14px", background: `rgba(${cor},0.15)`,
...
function PedidoCard({ pedido, isNovo, onAtualizarStatus }) {
  ...
  return (
    <div style={{
      background: isNovo ? "rgba(236,72,153,0.08)" : "rgba(255,255,255,0.03)",
      border: `1px solid ${isNovo ? "rgba(236,72,153,0.6)" : "rgba(255,255,255,0.07)"}`,
      borderRadius: "18px", overflow: "hidden", transition: "all 0.3s",
      animation: isNovo ? "glowBorder 1s infinite" : "none"
    }}>
      <div onClick={() => setExpandido(!expandido)} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", marginTop: "2px" }}>📍 {pedido.endereco}</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <div style={{
            background: `rgba(${status.cor},0.15)`,
            border: `1px solid rgba(${status.cor},0.4)`,
            color: `rgb(${status.cor})`,
      ...
      {expandido && (
        <div style={{ padding: "0 24px 20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ paddingTop: "16px", marginBottom: "12px", fontSize: "12px", color: "rgba(255,255,255,0.3)", letterSpacing: "1px" }}>
            ITENS DO PEDIDO
          </div>
          {pedido.itens?.map((item, i) => (
            <div key={i} style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", fontSize: "14px"
            }}>
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            {pedido.status === "Pendente" && (
              <>
                <BotaoAcao
                  label={carregando ? "Aguarde..." : "🔴 Iniciar Preparo"}
                  cor="236,72,153"
                  onClick={() => handleStatus("EmPreparo")}
                  disabled={carregando}
                />
                <BotaoAcao
                  label="⚫ Cancelar"
            ...
            {pedido.status === "EmPreparo" && (
              <BotaoAcao
                label={carregando ? "Aguarde..." : "🟢 Pedido Pronto — Marcar Entregue"}
                cor="34,197,94"
            ...
            {pedido.status === "Entregue" && (
              <div style={{ fontSize: "13px", color: "rgba(34,197,94,0.7)", padding: "8px 0" }}>
                ✅ Pedido finalizado e entregue
              </div>
            )}
            {pedido.status === "Cancelado" && (
              <div style={{ fontSize: "13px", color: "rgba(239,68,68,0.7)", padding: "8px 0" }}>
                ❌ Pedido cancelado
              </div>
            )}
          </div>
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: "all 0.2s",
        background: outline
          ? hovered ? `rgba(${cor},0.15)` : "transparent"
          : hovered ? `rgba(${cor},0.8)` : `rgba(${cor},0.2)`,
        border: `1px solid rgba(${cor},0.5)`,
        color: `rgb(${cor})`,
        boxShadow: hovered && !outline ? `0 4px 15px rgba(${cor},0.3)` : "none",
