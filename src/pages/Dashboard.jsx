...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f0f0f', fontFamily: "'Georgia', serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 40px', borderBottom: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(15,15,15,0.92)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <img src={logo} alt="Delícias da Mila" style={{ width: '56px', height: '56px', borderRadius: '16px', objectFit: 'cover', objectPosition: 'top', border: '2px solid rgba(236,72,153,0.5)', boxShadow: '0 4px 20px rgba(236,72,153,0.3)' }} />
          <div>
            <div style={{ fontSize: '18px', fontWeight: 'bold', letterSpacing: '0.5px', background: 'linear-gradient(135deg, #fff, #f9a8d4)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Delícias da Mila</div>
            <div style={{ fontSize: '11px', color: '#ec4899', letterSpacing: '2px', textTransform: 'uppercase' }}>Painel Administrativo</div>
          </div>
        </div>
      ...
      <main style={{ padding: '48px 40px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          <NavCard emoji="🍔" title="Produtos" desc="Gerencie o cardápio completo" color="#ec4899" onClick={() => navigate('/produtos')} badge="Ativo" />
          <NavCard emoji="📋" title="Cardápio" desc="Veja como os clientes enxergam" color="#f97316" onClick={() => navigate('/cardapio')} badge="Público" />
          <NavCard emoji="🛒" title="Pedidos" desc="Acompanhe em tempo real" color="#22c55e" onClick={() => navigate('/pedidos')} badge="Ao vivo" pulse />
        </div>

        <div style={{ marginTop: '60px', textAlign: 'center', fontSize: '12px', color: 'rgba(255,255,255,0.2)', letterSpacing: '1px' }}>
...
function NavCard({ emoji, title, desc, color, onClick, badge, pulse }) {
  ...
  const colorMap = { '#ec4899': '236,72,153', '#f97316': '249,115,22', '#22c55e': '34,197,94' }
  const rgb = colorMap[color] || '255,255,255'

  return (
    <div onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: hovered ? `linear-gradient(135deg, rgba(${rgb},0.2), rgba(${rgb},0.05))` : 'rgba(255,255,255,0.03)', border: `1px solid ${hovered ? color : 'rgba(255,255,255,0.08)'}`, borderRadius: '20px', padding: '32px', cursor: 'pointer', transition: 'all 0.3s ease', transform: hovered ? 'translateY(-4px)' : 'translateY(0)', boxShadow: hovered ? `0 20px 40px rgba(${rgb},0.2)` : 'none', position: 'relative' }}>
      <div style={{ position: 'absolute', top: '16px', right: '16px', background: `rgba(${rgb},0.15)`, border: `1px solid rgba(${rgb},0.4)`, color: color, fontSize: '10px', padding: '3px 10px', borderRadius: '20px', letterSpacing: '1px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: '5px' }}>
        {pulse && <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: color, display: 'inline-block', animation: 'pulse 1.5s infinite' }} />}
        {badge}
      </div>
      <div style={{ fontSize: '44px', marginBottom: '16px', display: 'inline-block', transition: 'transform 0.3s', transform: hovered ? 'scale(1.1)' : 'scale(1)' }}>{emoji}</div>
